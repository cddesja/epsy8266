...
### Question 1: Based on the plot, what sample size do you need to have a power of at least .80 to detect a correlation of .2 between X and Y? (1 pt)

### Question 2: Now copy all the code above and paste it below and then adapt the `pop.mod` code to determine the sample size you need to get a power of .8 to detect a correlation of .4 between X and Y and change the sample sizes you'll investigate to be from 10 to 100 by 10. Hint, this requires you to change two things in the code! Run the simulation below and then report the minimum sample size need to have a power of .80. (3 pts)
...
### Question 5: Does the relationship of X on Y have an impact on this? Conduct a power analysis by changing this relationship from from .3 to .6 for X on Y. Report the MDES that achieves a power of at least .80 for the z-test of the indirect effect. (3 pts)

### Question 6: Repeat this simulation but set the relationship from .6 to 1.  Report the minimum size of the indirect effect that achieves a power of at least .80. (3 pts)
...
### Question 10: Write the syntax for the correct lavaan model that you will fit based on the pop.mod and call that the model, correct.mod. (1 pt)
...
For the simulation you can ignore the warning message, but what does the warning message indicate? If this was your study, should you ignore this message? What might be the causes of this message? Just food for thought ...
...
### Question 13: Given the relationship you between power and sample size, do the results from Question 12 mirror this? What would you expect would have to type I error rate as sample size increases? (2 pt)
