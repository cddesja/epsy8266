...
- __The full APA citation of the article__.
- __A path diagram(s) from the article__. This should be a screenshot of their path diagram(s) copied/pasted into Word.
- A very brief __description of the model you will reproduce__. What hypotheses was it designed to test? 
- __Their results__. This could be a table(s) or a path diagram(s) and should be a screenshot from their article.
- __Being brief, describe if you were able to reproduce their results either completely, partially, or not at all__.
...
- __The title, authors, and journal of the paper__
- __The data__. The covariance structure, sample size, and mean structure (if necessary) or raw data. 
- __The lavaan syntax for writing the model__. Please try to use full notation and not a short cut (i.e., use `lavaan` and not the `cfa` or `sem` function).
- __The R code to fit the model and print the summaries need to reproduce their findings__ (i.e., if they reported standardized pattern coefficients and fit statistics your code should request this information, too.)
